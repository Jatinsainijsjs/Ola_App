...
//    }
    ...
    public ResponseEntity<?> bookTrip(@RequestBody TripRequest tripRequest) {
        try {
            ...
            if (tripDTO.getDistance()>2000){
                Map<String, String> response = new HashMap<>();
                // response.put("error", "Failed to book trip: " + e.getMessage());

                response.put("Hah! That sounds like a joke." ,"However, if it is serious I wish I could book a flight ticket for you. But unfortunately, we only provide road vehicles as of now.Thank you. You may try another destination which is reachable by road.");
